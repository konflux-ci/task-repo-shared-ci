...
1. `Syntax Validation (Always Runs)`: It performs a quick syntax check for every modified task. This provides fast feedback for basic errors.
2. `Integration Tests (Runs Conditionally)`: It spins up a full Konflux environment and runs integration tests only if a changed task **has a tests directory**. This saves significant time and resources when tests are not required.
